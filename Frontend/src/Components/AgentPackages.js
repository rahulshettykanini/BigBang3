...
{
  await fetch('https://localhost:7112/api/Packages/' + id + 'agepac').then(response => response.json()).then(data =>setac(data))

}

const AgentBookings = async ()=>
{
  await fetch('https://localhost:7112/api/BookingTrips/' + id + 'agent').then(response => response.json()).then(data =>setbc(data))

}

const AgentHotels = async ()=>
{
  await fetch('https://localhost:7112/api/Hotels/' + id + 'agenthotel').then(response => response.json()).then(data =>setcc(data))
...
<div>
    ...
    <div>
      <button onClick={AgentPackagess}>SEE AGENT PACKAGES</button>
      </div>
...
<div>
  <button onClick={AgentBookings}>SEE MY BOOKINGS</button>
  </div>
...
<div>
  <button onClick={AgentHotels}>SEE MY HOTEL</button>
  </div>
